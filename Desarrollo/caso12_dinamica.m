...
% ---------------------------------------------------------
x0 = [  0, 10,  5, 20, 10, 15, 15, 20, 30, 35, 40, 30, 40, 50, 60 ];
y0 = [  0,  0, 15,  0, 15, 15, 30, 30, 15, 15, 15,  0,  0, 15,  0 ];
...
% ---------------------------------------------------------
triangulos = [
    1,  2,  3;   % triangulo izquierdo base
    2,  4,  5;
    2,  5,  3;
    4,  6,  5;
    5,  6,  7;
    6,  7,  8;   % pico izquierdo
    5,  7,  6;
    9, 12, 19;   % zona central - ajustar segun figura
    9, 10,  8;
   10, 11,  8;
   11, 13, 12;
   12, 13,  9;
...
% ---------------------------------------------------------
% FUNCION ODE - HIPOTESIS a.i (NO LINEAL, deformaciones finitas)
% K recalculada en cada paso con coordenadas deformadas
% Fuerzas rotan con las barras
% CON amortiguador en nodo c (opcional, flag)
% ---------------------------------------------------------
function dz = ode_nolineal(t, z, x0, y0, Ni, Nj, E, A, P_val, ...
    ...
    % Fuerzas externas: P rota con los nodos (direccion de la barra en config deformada)
    % Simplificacion: P sigue siendo vertical (peso) -> misma direccion
    % Para "fuerzas que siguen la barra", se proyecta sobre la barra deformada.
    % En este caso la carga es una fuerza puntual vertical -> mantiene -y
    F_glob = zeros(nDOF,1);
    F_glob(2*7)  = -P_val;
    F_glob(2*10) = -P_val;
...
% ---------------------------------------------------------
% FUNCION: detectar primer tF (cambio de signo en area de triangulos)
% ---------------------------------------------------------
...
% ---------------------------------------------------------
% FUNCION: calcular tension normal en barra e dado u_glob
% sigma = E * (L_actual - L0) / L0
% ---------------------------------------------------------
function [sigma, tau] = tension_barra(e, u_glob, x0, y0, Ni, Nj, E, L0b)
    nDOF = length(u_glob);
    ni = Ni(e); nj = Nj(e);
    dx0 = x0(nj)-x0(ni); dy0 = y0(nj)-y0(ni);

    % Desplazamientos actuales
    u_glob_full = u_glob;
    x_act_ni = x0(ni) + u_glob_full(2*ni-1);
    y_act_ni = y0(ni) + u_glob_full(2*ni);
    x_act_nj = x0(nj) + u_glob_full(2*nj-1);
    y_act_nj = y0(nj) + u_glob_full(2*nj);
